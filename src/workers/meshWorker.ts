import { WorkerRequest, MeshEngineResult, WasmLithoModule, LithoParams, ProgressCallback, CMYWChannel, COLOR_CHANNELS, PaletteMeshEntry } from './types';
import { generateExtrusion } from './extrusionEngine';
import { generateColorLitho } from './colorLithoEngine';
import type { PrintConfig } from '../models/filamentPalette';

// --- WASM engine (loaded lazily, required) ---
let wasmModule: WasmLithoModule | null = null;

async function loadWasm(): Promise<WasmLithoModule> {
  if (wasmModule) return wasmModule;
  const wasm = await import('litho-engine-wasm/litho_engine_wasm.js');
  await wasm.default();
  wasmModule = wasm as unknown as WasmLithoModule;
  console.log('[LithoApp] WASM engine loaded successfully');
  return wasmModule;
}

/**
 * Run the lithophane pipeline via WASM.
 * Normals are computed in Rust (area-weighted vertex normals) — pure WASM performance.
 */
function runLithophaneWasm(
  wasm: WasmLithoModule,
  imageData: ImageData,
  width: number,
  height: number,
  params: LithoParams,
  postProgress: ProgressCallback
): MeshEngineResult {
  const result = wasm.generate_lithophane(
    imageData.data,
    width,
    height,
    params,
    postProgress
  );

  return {
    positions: result.positions,
    indices: result.indices,
    normals: result.normals,
    uvs: result.uvs,
    stats: result.stats,
  };
}

// Main WebWorker entry point
self.onmessage = async function (e: MessageEvent<WorkerRequest>) {
  const { id, mode, imageData, width, height, params } = e.data;

  const postProgress = (progress: number, message: string) => {
    self.postMessage({ type: 'progress', id, progress, message });
  };

  try {
    // Load WASM engine (fast no-op after first call)
    const wasm = await loadWasm();

    switch (mode) {
      case 'lithophane': {
        const result = runLithophaneWasm(wasm, imageData, width, height, params, postProgress);
        postMeshResult(id, result);
        return;
      }
      case 'extrusion': {
        const result = generateExtrusion(imageData, width, height, params, postProgress, wasm);
        postMeshResult(id, result);
        return;
      }
      case 'color-litho': {
        // Generate 4 separate meshes (C, M, Y, W)
        const colorMeshSet = generateColorLitho(imageData, width, height, params, postProgress, wasm);

        // Collect all ArrayBuffers for transfer
        const transferables: Transferable[] = [];
        for (const ch of COLOR_CHANNELS) {
          const mesh = colorMeshSet[ch];
          transferables.push(
            mesh.positions.buffer as ArrayBuffer,
            mesh.indices.buffer as ArrayBuffer,
            mesh.normals.buffer as ArrayBuffer,
          );
          if (mesh.uvs) transferables.push(mesh.uvs.buffer as ArrayBuffer);
          if (mesh.thickness) transferables.push(mesh.thickness.buffer as ArrayBuffer);
        }

        self.postMessage(
          { type: 'color-complete', id, colorMeshSet },
          { transfer: transferables }
        );
        return;
      }
      case 'encode-stl': {
        // STL encoding runs entirely in WASM — zero main-thread blocking
        const stlPos = e.data.stlPositions!;
        const stlIdx = e.data.stlIndices!;
        const stlBuffer = wasm.encode_stl(stlPos, stlIdx);
        self.postMessage(
          { type: 'stl-complete', id, stlBuffer },
          { transfer: [stlBuffer.buffer as ArrayBuffer] }
        );
        return;
      }
      case 'palette-litho': {
        // Generate one mesh per filament using calibrated palette
        // Lazy-load to avoid bloating the worker bundle for basic litho/extrusion
        const { generatePaletteLitho } = await import('./colorLithoEngine');
        const printConfig: PrintConfig = JSON.parse(e.data.printConfigJson!);
        const paletteMeshSet = await generatePaletteLitho(
          imageData, width, height, params, printConfig, postProgress, wasm
        );

        // Collect all ArrayBuffers for transfer
        const paletteTransferables: Transferable[] = [];
        for (const entry of paletteMeshSet.entries) {
          const m = entry.mesh;
          paletteTransferables.push(
            m.positions.buffer as ArrayBuffer,
            m.indices.buffer as ArrayBuffer,
            m.normals.buffer as ArrayBuffer,
          );
          if (m.uvs) paletteTransferables.push(m.uvs.buffer as ArrayBuffer);
          if (m.thickness) paletteTransferables.push(m.thickness.buffer as ArrayBuffer);
        }

        self.postMessage(
          { type: 'palette-complete', id, paletteMeshSet },
          { transfer: paletteTransferables }
        );
        return;
      }
      case 'encode-stl-pack': {
        // Encode all 5 CMYW channels as separate STL buffers, then ZIP
        const { zipSync } = await import('fflate');
        const pack = e.data.stlPack!;
        const zipEntries: Record<string, Uint8Array> = {};

        // Numbered filenames force print order in any slicer
        const STL_FILENAMES: Record<typeof COLOR_CHANNELS[number], string> = {
          white:     '1_white_base.stl',
          yellow:    '2_yellow.stl',
          magenta:   '3_magenta.stl',
          cyan:      '4_cyan.stl',
          white_top: '5_white_top.stl',
        };

        for (const ch of COLOR_CHANNELS) {
          const { positions, indices } = pack[ch];
          const stl = wasm.encode_stl(positions, indices);
          zipEntries[STL_FILENAMES[ch]] = stl;
        }

        // Include print instructions
        const readme = new TextEncoder().encode(
          'COLOR LITHOPHANE — PRINT INSTRUCTIONS\n' +
          '======================================\n\n' +
          'Print order (bottom to top):\n' +
          '  1. 1_white_base.stl  → WHITE filament (luminosity layer)\n' +
          '  2. 2_yellow.stl      → YELLOW filament (color filter)\n' +
          '  3. 3_magenta.stl     → MAGENTA filament (color filter)\n' +
          '  4. 4_cyan.stl        → CYAN filament (color filter)\n' +
          '  5. 5_white_top.stl   → WHITE filament (top diffuser)\n\n' +
          'Recommended slicer settings:\n' +
          '  - Layer height: 0.1mm (first layer: 0.15-0.2mm)\n' +
          '  - Infill: 100% rectilinear\n' +
          '  - Nozzle: 0.2mm recommended (0.4mm works)\n' +
          '  - Import all 5 STL files as a single multi-part object\n' +
          '  - Assign filaments: layers 1,5 = White PLA; 2 = Yellow; 3 = Magenta; 4 = Cyan\n\n' +
          'Generated by LithoApp\n'
        );
        zipEntries['README.txt'] = readme;

        const zipBuffer = zipSync(zipEntries, { level: 1 }); // fast compression
        self.postMessage(
          { type: 'stl-pack-complete', id, zipBuffer },
          { transfer: [zipBuffer.buffer as ArrayBuffer] }
        );
        return;
      }
      case 'encode-palette-stl-pack': {
        // Encode per-filament meshes as numbered STLs, then ZIP
        const { zipSync: zipSyncPalette } = await import('fflate');
        const paletteEntries = e.data.palettePack!;
        const paletteZipEntries: Record<string, Uint8Array> = {};

        for (let fi = 0; fi < paletteEntries.length; fi++) {
          const entry = paletteEntries[fi];
          const { positions, indices } = entry.mesh;
          const stl = wasm.encode_stl(positions, indices);
          // Numbered filenames force print order; sanitise filament name for filename
          const safeName = entry.filamentName.replace(/[^a-zA-Z0-9_-]/g, '_').toLowerCase();
          paletteZipEntries[`${fi + 1}_${safeName}.stl`] = stl;
        }

        // Include print instructions with filament mapping
        const paletteReadme = new TextEncoder().encode(
          'PALETTE LITHOPHANE — PRINT INSTRUCTIONS\n' +
          '========================================\n\n' +
          'Print order (bottom to top):\n' +
          paletteEntries.map((e, i) =>
            `  ${i + 1}. ${i + 1}_${e.filamentName.replace(/[^a-zA-Z0-9_-]/g, '_').toLowerCase()}.stl → ${e.filamentName} (${e.filamentHex})\n`
          ).join('') +
          '\nRecommended slicer settings:\n' +
          '  - Layer height: 0.1mm (first layer: 0.15-0.2mm)\n' +
          '  - Infill: 100% rectilinear\n' +
          '  - Nozzle: 0.2mm recommended (0.4mm works)\n' +
          '  - Import all STL files as a single multi-part object\n' +
          '  - Assign filaments per the order above\n\n' +
          'Generated by LithoApp\n'
        );
        paletteZipEntries['README.txt'] = paletteReadme;

        const paletteZipBuffer = zipSyncPalette(paletteZipEntries, { level: 1 });
        self.postMessage(
          { type: 'stl-pack-complete', id, zipBuffer: paletteZipBuffer },
          { transfer: [paletteZipBuffer.buffer as ArrayBuffer] }
        );
        return;
      }
      case 'cookie-cutter':
        throw new Error('Cookie Cutter mode not yet implemented');
      default:
        throw new Error(`Unknown generation mode: ${mode}`);
    }

  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'An error occurred during mesh generation';
    self.postMessage({
      type: 'error',
      id,
      message
    });
  }
};

/** Helper to post a single mesh result with proper transferables */
function postMeshResult(id: number, result: MeshEngineResult) {
  const transferables: Transferable[] = [
    result.positions.buffer as ArrayBuffer,
    result.indices.buffer as ArrayBuffer,
    result.normals.buffer as ArrayBuffer,
  ];
  if (result.uvs) transferables.push(result.uvs.buffer as ArrayBuffer);
  if (result.thickness) transferables.push(result.thickness.buffer as ArrayBuffer);

  self.postMessage({
    type: 'complete',
    id,
    positions: result.positions,
    indices: result.indices,
    normals: result.normals,
    uvs: result.uvs,
    thickness: result.thickness,
    stats: result.stats
  }, { transfer: transferables });
}

